// Constantes générales du programme FlexFi
...
// Prix du minting NFT
pub const NFT_MINT_COST: u64 = 20_000_000; // 20 USDC (avec 6 décimales)

// Configurations de carte (APR, frais BNPL, échéances)
pub struct CardConfig {
    pub apr_percentage: u16,           // APR en points de base (ex: 400 = 4%)
    pub bnpl_fee_percentage: u16,      // Frais BNPL en points de base
    pub bnpl_fee_12months: u16,        // Frais BNPL pour 12 mois
    pub max_installments: u8,          // Nombre max d'échéances
    pub available_installments: [u8; 4], // Échéances disponibles (3,4,6,12)
    pub cashback_percentage: u16,      // Cashback en points de base
    pub cashback_limit: u64,           // Limite mensuelle de cashback en USDC (avec 6 décimales)
    pub nft_cost: u64,                 // Coût du NFT en USDC (avec 6 décimales)
}

// Obtenir la configuration d'une carte
pub fn get_card_config(card_type: u8) -> CardConfig {
    match card_type {
        CARD_STANDARD => CardConfig {
            apr_percentage: 400,           // 4%
            bnpl_fee_percentage: 700,      // 7%
            bnpl_fee_12months: 700,        // 7% pour 12 mois aussi
            max_installments: 6,           // 6 mois max
            available_installments: [3, 4, 6, 0],
            cashback_percentage: 0,        // Pas de cashback
            cashback_limit: 0,             // Pas de limite
            nft_cost: 0,                   // Standard n'a pas de NFT inclus
        },
        CARD_SILVER => CardConfig {
            apr_percentage: 500,           // 5%
            bnpl_fee_percentage: 400,      // 4%
            bnpl_fee_12months: 700,        // 7% pour 12 mois
            max_installments: 12,          // 12 mois max
        ...
        CARD_GOLD => CardConfig {
            ...
            bnpl_fee_12months: 500,        // 5% pour 12 mois
            max_installments: 12,          // 12 mois max
            available_installments: [3, 4, 6, 12],
        ...
        CARD_PLATINUM => CardConfig {
            ...
            bnpl_fee_12months: 300,        // 3% pour 12 mois aussi
            max_installments: 12,          // 12 mois max
            available_installments: [3, 4, 6, 12],
            cashback_percentage: 150,      // 1.5%
            cashback_limit: 300_000_000,   // 300 USDC
            nft_cost: 0,                   // NFT inclus
        },
        _ => CardConfig {                  // Valeur par défaut (Standard)
            apr_percentage: 400,
...
// Obtenir le gain d'APR pour une combinaison avec NFT
...
// Obtenir les frais de pénalité de retard selon la combinaison carte+NFT
pub fn get_late_payment_penalty(card_type: u8, nft_type: u8) -> u16 {
    match (card_type, nft_type) {
        ...
        _ => 1000,                         // 10% par défaut
    }
}

// Constantes liées au BNPL
...
// Constantes liées au staking
pub const MIN_STAKING_AMOUNT: u64 = 10_000_000; // 10 USDC (avec 6 décimales)
pub const MIN_STAKING_LOCK_DAYS: u16 = 7;
pub const MAX_STAKING_LOCK_DAYS: u16 = 365;

// Constantes liées au scoring
...
// Liste blanche des wallets en dur pour les tests
pub const HARDCODED_WHITELIST: &[&str] = &[
